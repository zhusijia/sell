		...
		<div class="goods-wrapper" ref="goodsWrapper">
			<div>
				<div v-for="item in goods" class="food-list food-list-hook">
					...
					<ul>
						<li @click="selectFood(food, $event)" v-for="food in item.foods" class="food-detail border-1px">
							<div class="icon">
...
export default {
  ...
  computed: {
    ...
    selectFoods() {
      let foods = [];
      _.each(this.goods, good => {
        _.each(good.foods, food => {
          if (food.count) {
            foods.push(food);
          }
        });
  ...
  methods: {
    ...
    selectMenu(index, event) {
      let goodsList = this.$refs.goodsWrapper.getElementsByClassName(
        "food-list-hook"
      );
